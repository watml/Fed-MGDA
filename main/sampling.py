...
def noniid_adult(dataset, num_users=100, num_vecs=100):
    ...
    :return:
    """
    num_shards = int(len(dataset)/num_vecs)
    idx_shard = [i for i in range(num_shards)]
    dict_users = {i: np.array([]) for i in range(num_users)}
    included = num_shards*num_vecs  # included samples
    idxs = np.arange(included)
    labels = dataset[:included][1].numpy()  # labels 
    ...
    for i in range(num_users):
        ...
        for rand in rand_set:
            dict_users[i] = np.concatenate(
                (dict_users[i], idxs[rand*num_vecs:(rand+1)*num_vecs]), axis=0)
    return dict_users
